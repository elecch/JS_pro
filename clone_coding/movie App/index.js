const apiKey = "6cb4abc3e11123e7496c55bff246182f";
...
//Clear resultelement for search
...
async function loadMoreResults(){
  ...
  const url = searchTerm ? `${searchUrl}${searchTerm}&page=${page}` : `http://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=${apiKey}&page=${page}`;
...
async function init(){
  clearResults();
  const url = `http://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=${apiKey}&page=${page}`;
  isSearching = false;
  await fetchAndShowResult(url);
}
